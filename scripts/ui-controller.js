...
const UIController = {
    ...
    renderNavigation: function() {
        ...
        DataStore.navigation.forEach((navItem, index) => {
            ...
            // 检查是否有对应的卡片组数据
            const cardGroupData = DataStore.cards[navItem.id];
            if (cardGroupData && cardGroupData.length > 0) {
                navElement.classList.add('has-submenu');
                ...
                // 添加二级菜单项
                cardGroupData.forEach(group => {
                    const submenuItem = document.createElement('div');
                    submenuItem.className = 'nav-item submenu-item';
                    submenuItem.textContent = group.groupName;
                    submenuItem.setAttribute('data-type', navItem.id);
                    submenuItem.setAttribute('data-group', group.groupName);
                    
                    // 为二级菜单项添加点击事件
                    submenuItem.addEventListener('click', (e) => {
                        e.stopPropagation(); // 阻止事件冒泡
                        this.handleSubmenuItemClick(submenuItem, group.groupName);
                    });
     ...
     * 处理二级菜单项点击
     */
    handleSubmenuItemClick: function(submenuItem, groupName) {
        // 移除所有二级菜单项的活动状态
        ...
        // 滚动到对应的分组
        const cardType = submenuItem.getAttribute('data-type');
        const targetGroup = document.querySelector(`.business-group[data-card-type="${cardType}"] .business-group-title[data-text="${groupName}"]`);
        if (targetGroup) {
            // 获取内容区域容器
            ...
            // 计算需要滚动的位置
            const scrollTop = contentArea.scrollTop + targetRect.top - contentRect.top - headerHeight - 20;
    ...
    handleNavItemClick: function(event) {
        ...
        // 移除所有导航项的激活状态和折叠所有子菜单
        this.navItems.forEach(nav => {
            nav.classList.remove('active');
            // 找到对应的子菜单并折叠
            const wrapper = nav.closest('.nav-item-wrapper');
            if (wrapper) {
        ...
        // 展开当前点击项的子菜单
        const currentWrapper = clickedItem.closest('.nav-item-wrapper');
        if (currentWrapper) {
            const currentSubmenu = currentWrapper.querySelector('.nav-submenu');
            if (currentSubmenu) {
